...
export async function rotateKey(pool, cmd) {
  ...
  try {
    return await withTransaction(pool, async (client) => {
      ...
      // device reuse never reaches the rotation work.
      let replay = await replayAdminCommand(client, {
        commandId, kind: 'rotate', requestHash: fullHash,
      });
      ...
      await persistAdminCommand(client, {
        commandId, deviceId, kind: 'rotate', requestHash: fullHash, response,
      });
      return { ...response, replayed: false };
    });
  } catch (e) {
    // Cross-device / cross-instance contention for the same commandId: the
    // loser transaction has rolled back (no partial rotation); answer from the
    // committed winner.
