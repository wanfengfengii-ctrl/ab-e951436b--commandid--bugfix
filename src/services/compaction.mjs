...
import {
  replayAdminCommand, persistAdminCommand, handleAdminCommandRace,
} from './admin_commands.mjs';
...
export async function compactDevice(pool, serverKey, cfg, deviceId, desiredCutoff = null, commandId = null) {
  ...
  try {
    return await withTransaction(pool, async (client) => {
      if (commandId !== null) {
        // Global reservation check up front: a commandId already owned by any
        // admin command (including another compaction kind/shape) never reaches
        // the checkpoint work.
        let replay = await replayAdminCommand(client, {
          commandId, kind: 'compact', requestHash,
        });
      ...
      if (commandId !== null) {
        // Re-check after the device lock: same-device commands serialize here,
        // so a winner that committed in the gap is always observed.
        const replay = await replayAdminCommand(client, {
          commandId, kind: 'compact', requestHash,
        });
        if (replay) return replay;
      }
      ...
      if (desiredCutoff === null) {
        ...
        if (cutoff <= prevSeq) {
          ...
          if (commandId !== null) {
            await persistAdminCommand(client, {
              commandId, deviceId, kind: 'compact', requestHash, response: skipped,
            });
          }
          return skipped;
        }
      ...
      if (commandId !== null) {
        await persistAdminCommand(client, {
          commandId, deviceId, kind: 'compact', requestHash, response: result,
        });
      }
      return result;
    });
  } catch (e) {
    // Only commands carrying a commandId can collide on the global
    // reservation; background compaction (commandId === null) never reaches
    // this arbitration.
    if (commandId === null) throw e;
    return handleAdminCommandRace(pool, e, {
      commandId, kind: 'compact', requestHash,
    });
  }
}
